# -*- encoding: utf-8 -*-
# stub: allison 2.0.3 ruby lib

Gem::Specification.new do |s|
  s.name = "allison".freeze
  s.version = "2.0.3"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Evan Weaver".freeze]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2008-01-21"
  s.description = "A modern, pretty RDoc template.".freeze
  s.email = "".freeze
  s.executables = ["allison".freeze]
  s.files = ["bin/allison".freeze]
  s.homepage = "http://blog.evanweaver.com/pages/code#allison".freeze
  s.rubyforge_project = "fauna".freeze
  s.rubygems_version = "2.6.10".freeze
  s.summary = "A modern, pretty RDoc template.".freeze

  s.installed_by_version = "2.6.10" if s.respond_to? :installed_by_version
end
